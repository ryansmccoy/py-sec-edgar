...
import py_sec_edgar.filing as edgar_filing
...
def prepare_filepaths(feed_item):
    ...
    feed_item['filing_path'] = os.path.join(feed_item['cik_directory'], os.path.basename(feed_item['Filename']))
...
def download_filing(feed_item, zip_contents=True):
    ...
    # shutil.make_archive(feed_item['filing_path'], 'zip', feed_item['cik_directory'])
