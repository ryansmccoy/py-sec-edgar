...
def query_db_for_filings_data(date_qtr, form_filter=None, cik=None, LATEST_VAR=False):
    ...
    list_all_filings = []
    conn_filing_master = sqlite3.connect(
        os.path.join(SSD_DATA_DIR, "FILINGS_MASTER.DB"))

    if date_qtr == "LATEST" or LATEST_VAR == True:
        df_frame = pd.read_sql(
            "SELECT * FROM TABLE_MASTER", conn_filing_master)
    else:
        df_frame = pd.read_sql(
            "SELECT * FROM TABLE_{}_{}_MASTER".format(date_qtr[0], date_qtr[1]), conn_filing_master)
...
def load_idx_files_from_sqldb(start_year=None, end_year=None):
    ...
    master_files = walk_dir_fullfilename(
        SEC_FULL_INDEX_DIR, contains="master.idx")
    ...
    for year in years:
        ...
        for year_file in years_files:
            print(year_file)
            table_name = year_file.split(
                "full-index\\")[1].replace("\\", "_").upper().split(".")[0]
            df_frame = pd.read_sql(
                "SELECT * FROM TABLE_{}".format(table_name), conn)
            frame = frame.append(df_frame)

    frame = frame.drop_duplicates(subset=['FILENAME'])
    frame = frame[frame['FORM_TYPE'] == "10-K"]
    frame["BASENAME"] = frame["FILENAME"].apply(
