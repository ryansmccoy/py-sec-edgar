...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logs/sec_edgar_monthly.log')  # File output
    ]
)
...
@click.option('--months-back', default=1, help='Number of months back to process (default: 1)')
def main(ticker_list, form_list, months_back):
    logger.info("Starting SEC EDGAR monthly XBRL data processing...")
    logger.info(f"Parameters: ticker_list={ticker_list}, form_list={form_list}, months_back={months_back}")

    # Download only the specific monthly XBRL RSS feeds needed based on months_back
    logger.info(f"Starting selective monthly XBRL feed download for last {months_back} months...")
    ...
    for months_ago in range(months_back):
        target_date = datetime.now() - timedelta(days=months_ago * 30)  # Approximate months
        url, filepath = py_sec_edgar.feeds.monthly.generate_monthly_index_url_and_filepaths(target_date)
        ...
        if os.path.exists(filepath):
            ...
            try:
                ...
                if not df_monthly.empty:
                    df_monthly['Date Filed'] = pd.to_datetime(df_monthly['Date Filed'], errors='coerce')
                    df_monthly = df_monthly.dropna(subset=['Date Filed'])
                    monthly_filings.append(df_monthly)
                    logger.info(f"Loaded {len(df_monthly)} XBRL filings from {filepath}")
    ...
    df_merged_monthly = pd.concat(monthly_filings, ignore_index=True).sort_values("Date Filed", ascending=False)
    ...
    if form_list:
        ...
        df_merged_monthly = df_merged_monthly[df_merged_monthly['Form Type'].isin(settings.forms_list)]
    ...
    if ticker_list:
        ...
        df_merged_monthly['CIK'] = pd.to_numeric(df_merged_monthly['CIK'], errors='coerce')
    ...
    if 'Link' in df_filings.columns and not df_filings['Link'].isna().all():
        ...
        df_filings['url'] = df_filings['Filename'].apply(lambda x: urljoin(settings.edgar_archives_url, x) if pd.notna(x) else '')
