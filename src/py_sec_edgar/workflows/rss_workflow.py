...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logs/sec_edgar_rss.log')  # File output
    ]
)
...
import py_sec_edgar.feeds.recent_filings
...
@click.option('--count', default=100, help='Number of recent filings to fetch (default: 100)')
@click.option('--form-type', default=None, help='Specific form type to filter (e.g., 8-K, 10-K)')
...
@click.option('--list-only', is_flag=True, help='Just list the RSS filings without processing them')
@click.option('--save-to-file', default=None, help='Save RSS data to file (e.g., rss_data.json)')
@click.option('--load-from-file', default=None, help='Load RSS data from file instead of fetching')
@click.option('--query-cik', default=None, help='Filter by specific CIK when loading from file')
@click.option('--query-ticker', default=None, help='Filter by specific ticker when loading from file')
@click.option('--query-form', default=None, help='Filter by specific form when loading from file')
@click.option('--show-entries', is_flag=True, help='Show detailed RSS entries with all fields')
@click.option('--search-text', default=None, help='Search for text in company names or descriptions')
def main(ticker_list, form_list, count, form_type, download, extract, list_only, save_to_file, load_from_file, query_cik, query_ticker, query_form, show_entries, search_text):
    logger.info("Starting SEC EDGAR RSS (recent) data processing...")
    logger.info(f"Parameters: ticker_list={ticker_list}, form_list={form_list}, count={count}, form_type={form_type}")
    logger.info(f"Options: download={download}, extract={extract}, list_only={list_only}")
    logger.info(f"File operations: save_to_file={save_to_file}, load_from_file={load_from_file}")
    logger.info(f"Query filters: query_cik={query_cik}, query_ticker={query_ticker}, query_form={query_form}")
    logger.info(f"Display options: show_entries={show_entries}, search_text={search_text}")
    ...
    if load_from_file:
        ...
        try:
            ...
            logger.info(f"Loaded {len(recent_filings_data)} filings from {load_from_file}")
        ...
        recent_feed = py_sec_edgar.feeds.recent_filings.RecentFilingsFeed()
        ...
        try:
            ...
            recent_filings_data = loop.run_until_complete(
                recent_feed.fetch_recent_filings(count=count, form_type=form_type)
            )
    ...
    if load_from_file and (query_cik or query_ticker or query_form or search_text):
        ...
        if query_cik:
            ...
            df_recent_filings = df_recent_filings[df_recent_filings['cik'].astype(str).str.contains(query_cik, case=False, na=False)]
        
        if query_ticker:
            logger.info(f"Filtering by ticker: {query_ticker}")
            df_recent_filings = df_recent_filings[df_recent_filings['ticker'].astype(str).str.contains(query_ticker, case=False, na=False)]
        
        if query_form:
            logger.info(f"Filtering by form type: {query_form}")
            df_recent_filings = df_recent_filings[df_recent_filings['form_type'].astype(str).str.contains(query_form, case=False, na=False)]
        
        if search_text:
            logger.info(f"Searching for text: {search_text}")
            # Search in company_name and title fields
            company_matches = df_recent_filings['company_name'].astype(str).str.contains(search_text, case=False, na=False)
            title_matches = df_recent_filings.get('title', pd.Series(dtype=str)).astype(str).str.contains(search_text, case=False, na=False)
            description_matches = df_recent_filings.get('description', pd.Series(dtype=str)).astype(str).str.contains(search_text, case=False, na=False)
            df_recent_filings = df_recent_filings[company_matches | title_matches | description_matches]
        
        logger.info(f"Query filtering: {original_count} -> {len(df_recent_filings)} filings")
    ...
    if save_to_file:
        ...
        try:
            ...
            logger.info(f"✅ Saved {len(recent_filings_data)} filings to {save_to_file}")
    ...
    if list_only:
        ...
        if show_entries:
            ...
            for i, filing in enumerate(df_recent_filings.itertuples(), 1):
                ...
                click.echo(f"{i:3d}. {form_type:8s} | {company[:50]:50s}{ticker_str} | CIK: {cik} | {filed_date}")
    ...
    if 'date_filed' in df_recent_filings.columns:
        df_recent_filings['Date Filed'] = pd.to_datetime(df_recent_filings['date_filed'], errors='coerce')
    ...
    for old_col, new_col in column_mapping.items():
        if old_col in df_recent_filings.columns and new_col not in df_recent_filings.columns:
    ...
    if 'filing_url' in df_recent_filings.columns and 'Filename' not in df_recent_filings.columns:
        ...
        df_recent_filings['Filename'] = df_recent_filings['filing_url'].apply(extract_filename_from_url)
        logger.info("Created Filename column from filing_url for FilingProcessor compatibility")
    ...
    if not load_from_file and form_list and hasattr(settings, 'forms_list'):
        ...
        df_recent_filings = df_recent_filings[df_recent_filings['Form Type'].isin(settings.forms_list)]
    ...
    if not load_from_file and ticker_list and ticker_list.strip() and cik_list:
        ...
        df_recent_filings['CIK'] = pd.to_numeric(df_recent_filings['CIK'], errors='coerce')
        ...
        if load_from_file:
            logger.info(f"Loading from file - processing all {len(df_recent_filings)} filings without CIK filtering")
        else:
            logger.info(f"No CIK filtering applied - processing all {len(df_recent_filings)} filings")
    ...
    if 'url' not in df_recent_filings.columns:
        if 'Filename' in df_recent_filings.columns:
            ...
            logger.warning("No URL or Filename column found - filings may not be processable")
    ...
    filing_data_dir = str(settings.base_dir / "data" / "Archives" / "edgar" / "data" / "CIK" / "FOLDER")
    ...
    for i, sec_filing in df_filings.iterrows():
        logger.info(f"Processing filing {i+1}/{len(df_filings)}: {sec_filing.get('Form Type', 'Unknown')} for CIK {sec_filing.get('CIK', 'Unknown')}")
