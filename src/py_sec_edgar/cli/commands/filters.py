...
from py_sec_edgar.cli.common import standard_ticker_options, standard_form_options
...
@click.option('--days-back', type=int, default=730, show_default=True, help='How many days back to load when dates not provided (~2 years).')
@standard_ticker_options
@standard_form_options
@click.option('--limit', type=int, default=50, show_default=True, help='Limit rows for display/output')
@click.option('--json-output/--table-output', default=False, show_default=True, help='Print JSON summary instead of table')
@click.option('--include-urls/--no-urls', default=False, show_default=True, help='Include filing URLs in output')
@click.option('--save-csv', type=click.Path(), default=None, help='Save filtered results to CSV file')
@click.option('--interactive/--no-interactive', default=False, show_default=True, help='Interactive parameter selection')
def filter_daily(
    start_date: str | None,
    end_date: str | None, 
    days_back: int,
    ...
    if interactive:
        start_date = click.prompt('Start date (YYYY-MM-DD, empty to skip)', default=start_date or '', type=str) or None
        end_date = click.prompt('End date (YYYY-MM-DD, empty to skip)', default=end_date or '', type=str) or None
    ...
    py_sec_edgar.feeds.daily.update_daily_files(start_date=start_date, end_date=end_date, days_back=days_back)
    ...
    df = pd.concat(daily_frames, ignore_index=True).sort_values('Date Filed', ascending=False)
    logger.info(f"📈 Loaded {len(df)} daily filing records")

    # Apply filtering
    df = _apply_filters(df, df_cik_tickers, tickers, ticker_file, no_ticker_filter, 
                       forms, form, form_file, no_form_filter)

    # Generate URLs if requested
    if include_urls:
        df = df.assign(url=df['Filename'].apply(lambda x: urljoin(settings.edgar_archives_url, x)))
...
def filter_full_index(
    ...
    # Load data
    df_cik_tickers = pd.read_csv(str(settings.cik_tickers_csv))
    df_idx = pq.read_table(str(settings.merged_idx_filepath)).to_pandas().sort_values('Date Filed', ascending=False)
    
    logger.info(f"📈 Loaded {len(df_idx)} full index records")

    # Apply filtering
    df_idx = _apply_filters(df_idx, df_cik_tickers, tickers, ticker_file, no_ticker_filter,
                           forms, form, form_file, no_form_filter)

    # Generate URLs if requested
    if include_urls:
        df_idx = df_idx.assign(url=df_idx['Filename'].apply(lambda x: urljoin(settings.edgar_archives_url, x)))
...
@click.option('--months-back', type=int, default=24, show_default=True, help='How many months back to include (~2 years)')
@standard_ticker_options
@standard_form_options
@click.option('--limit', type=int, default=50, show_default=True, help='Limit rows for display/output (0 for all)')
@click.option('--json-output/--table-output', default=False, show_default=True, help='Print JSON summary instead of table')
@click.option('--include-urls/--no-urls', default=False, show_default=True, help='Include filing URLs in output')
@click.option('--save-csv', type=click.Path(), default=None, help='Save filtered results to CSV file')
@click.option('--interactive/--no-interactive', default=False, show_default=True, help='Interactive parameter selection')
@click.option('--log-level', type=click.Choice(['DEBUG','INFO','WARNING','ERROR','CRITICAL'], case_sensitive=False), 
              default='INFO', show_default=True, help='Set logging level')
def filter_monthly_xbrl(
    ...
    if interactive:
        ...
        ticker_prompt = click.prompt('Ticker list (comma-separated, empty to skip)', default='', show_default=False)
        if ticker_prompt.strip():
            tickers = tuple(ticker_prompt.split(','))
        no_form_filter = not click.confirm('Apply form type filter?', default=not no_form_filter)
        ...
        if os.path.exists(filepath):
            try:
                ...
                if rows:
                    ...
                    dfm['Date Filed'] = pd.to_datetime(dfm['Date Filed'], errors='coerce')
    ...
    df = pd.concat(monthly_frames, ignore_index=True).sort_values('Date Filed', ascending=False)
    ...
    if include_urls:
        if 'Link' in df.columns and not df['Link'].isna().all():
            ...
            df['url'] = df['Filename'].apply(lambda x: urljoin(settings.edgar_archives_url, x) if pd.notna(x) else '')

    # Output results
    _output_results(df, limit if limit > 0 else len(df), json_output, save_csv, "monthly-xbrl")
...
def _apply_filters(
    ...
    if not no_ticker_filter:
        ...
        if ticker_file and ticker_file.exists():
            ...
            file_tickers = ticker_df.iloc[:, 0].astype(str).str.strip().str.upper().tolist()
            all_tickers.update(file_tickers)
        
        if all_tickers:
            # Filter CIK mapping by tickers
            df_symbols = df_cik_tickers[df_cik_tickers['SYMBOL'].astype(str).str.upper().isin(all_tickers)]
...
def _output_results(df: pd.DataFrame, limit: int, json_output: bool, save_csv: Path | None, data_type: str) -> None:
