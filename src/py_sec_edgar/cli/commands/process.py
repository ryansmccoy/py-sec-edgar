...
@click.option(
    "--cik",
    type=str,
    help="Process filings for specific CIK"
)
@click.option(
...
def process_filings(
    ...
    form_list = list(form_types) if form_types else settings.forms_list
    ...
    try:
        ...
        if cik:
            ...
            click.echo("💡 Try: uv run python -m py_sec_edgar workflows rss --query-cik {cik}")
        elif ticker_list:
            ...
            click.echo(f"💡 Try: uv run python -m py_sec_edgar workflows rss --ticker-file {ticker_list}")
        else:
            logger.info("Processing recent filings...")
            # Process recent filings (default behavior)
            click.echo("📊 Processing recent filings...")
            click.echo("⚠️  Recent filings processing is available via workflows commands")
            click.echo("💡 Try: uv run python -m py_sec_edgar workflows daily --days-back 7")
...
def process_monthly_xbrl(
    ...
    try:
        ...
        monthly_main(ticker_list=ticker_list, months_back=months_back, form_list=form_list)
