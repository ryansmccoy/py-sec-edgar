...
class SECEdgarSettings(BaseSettings):
    ...
    app_name: str = Field(default="SEC EDGAR Filing API", description="Application name")
    ...
    # Directory settings
    base_dir: Path = Field(default_factory=lambda: Path(__file__).parent.parent.parent, description="Project root directory")
    sec_data_dir: str | None = Field(default=None, description="SEC data directory path", alias="SEC_DATA_DIR")
    ...
    edgar_archives_url: str = Field(
        ...
        description="SEC EDGAR Archives URL"
    )
    ...
    request_delay: float = Field(
        ...
        alias="SEC_REQUEST_DELAY"
    )

    max_retries: int = Field(
        default=3, 
        description="Maximum retry attempts",
        alias="SEC_MAX_RETRIES"
    )
    ...
    index_start_date: str = Field(
        default_factory=lambda: (
            ...
            (datetime.now() - __import__('datetime').timedelta(days=730)).strftime("%-m/%-d/%Y")
            if os.name != 'nt' else
            (datetime.now() - __import__('datetime').timedelta(days=730)).strftime("%#m/%#d/%Y")
        ),
    ...
    index_end_date: str = Field(
        default_factory=lambda: datetime.now().strftime("%-m/%-d/%Y") if os.name != 'nt' else datetime.now().strftime("%#m/%#d/%Y"),
    ...
    index_files: list[str] = Field(
        default=["master.idx"],
        description="Index files to process per quarter"
    )
