...
def edgar_filing_idx_create_filename(item):
    ...
    edgfilepath = 'edgaridx-{}_txt_#2_{}_#3_{}_#4_{}_#5_({})_{}_#6_{}_#7_{}.txt'.format(
        str(item['edgar_filingdate'][0:6]), os.path.dirname(item['link'].replace(r'http://www.sec.gov/Archives/',"")).replace("/","_"), date_string, item['edgar_ciknumber'], item['ticker'], item['edgar_companyname'],item['edgar_formtype'], str(item['edgar_period']))
...
def celery_feed_all_filings_for_download(df_all_filings, celery_enabled=True):
    ...
    for ii, file in enumerate(df_all_filings.itertuples()):
        ...
        quarter = '{}'.format(datetime.strptime(file.DATE_FILED, '%Y-%m-%d').year), "QTR{}".format(int((datetime.strptime(file.DATE_FILED, '%Y-%m-%d').month - 1) / 3) + 1)
        item = {}
        ...
        url_quarter = 'edgar/data/{}/{}/{}'.format(file.CIK, os.path.basename(file.FILENAME).replace("-", "").replace(".txt", ""), os.path.basename(file.FILENAME))
        item['URL'] = urljoin(edgar_Archives_url, url_quarter)
        item['LOCAL'] = os.path.join('{}'.format(quarter[0]), "{}".format(quarter[1]),'{}'.format(os.path.basename(file.FILENAME)))
        ...
        if not os.path.exists(item['windows_output_filepath']):
            ...
            print("Filepath Already exists {}".format(item['windows_output_filepath']))
...
def main():
    ...
    for qtr_date in dates_quarters:
        ...
        df_all_filings = query_db_for_filings_data(qtr_date,form_filter=form_filter)
